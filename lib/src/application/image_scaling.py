...
def _build_resize_program(
    input_filepath: str, output_filename: str, scale: int
) -> Program:
...
def _jpeg_scaler_v1_handler(
    ...
    return _build_resize_program(
        input_filepath=input_filepath, output_filename=output_filename, scale=scale
    )
...
def _jpeg_scaler_v2_handler(datafile: str) -> List[Program]:
    try:
        io = RealIO()
        json_data = io.load_json_data_from_file(datafile)
